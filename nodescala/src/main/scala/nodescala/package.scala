...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs.foldRight(Future.successful(List.empty[T])) { (cur, acc) =>
        val p = Promise[List[T]]()

        cur.onComplete { tr1 =>
          acc.onComplete { tr2 =>
            (tr1, tr2) match {
              case (Success(v), Success(tail)) => p.success(v :: tail)
              case (Failure(e), _) => p.failure(e)
              case (_, Failure(e)) => p.failure(e)
            }
          }
        }
        p.future
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try Await.result(f, Duration.Zero)
        ...
        case _: TimeoutException => throw new NoSuchElementException("Future : time out")
      }
    }
